 ...
 */
class Social_model extends BaseModel
{
	// A list of all supported social networks
	 ...
	 *
	 */
	public function getSocial($email)
	{
	 ...
	 * @throws Exception
	 */
	public function findAccountIndexByUsername($network, $username) {
		$network = $this->getNetwork($network);
		if ($network != null) {
	...
	/**
	 * Updates just the username fields for all social platforms included in data.
	 *
	 * DOES NOT UPDATE $this->doc
	 *
	 * @param $data Array ex: array('facebook' => array('username' => 'jimbolia'))
	 */
	/*
	public function updateUserNames($data)
	{
	...
	*/
	 ...
	 * the username field. If an account with the same username
	...
	public function addOrUpdateAccount($data, $network)
	{
		...
		foreach ($this->doc['networks'][$network] as $index => $account) {
			if (isset($account['username']) && $account['username'] == $data['username']) {
				$accountIndex = $index;
			}
		}
	 ...
	 * @throws Exception
	 */
	public function disconnect($network, $accountIndex = 0)
	{
		if (!isset($this->email) || !isset($this->doc)) {
			throw new Exception('User not set');
		}
		...
		if (isset($this->doc['networks'][$network][$accountIndex])) {
			$this->doc['networks'][$network][$accountIndex]['connected'] = false;
	...
	*/
	...
	public function getReach($networkName, $accountIndex, $account = null)
	{
		...
		$followers = 0;
		if (isset($account['connected']) && $account['connected'] == true && isset($account['followers'])) {
